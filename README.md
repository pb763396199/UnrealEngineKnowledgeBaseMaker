...
| 查询记忆 | `query_memory_search` / `query_memory_validate` / `query_memory_replay` | 高 | 先搜已沉淀路线，也会返回自动审计里的历史 trace 片段；复用前必须校验并重跑当前事实。 |
...
| 复用历史业务路线 | `preflight` -> `query_memory_search` -> 看 `results` 和 `audit_traces` -> `query_memory_validate/query_memory_replay` -> 继续补查新增/变化缺口 |
